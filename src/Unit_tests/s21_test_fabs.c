...
static double normal_val[] = {1.0, -1.0, 0.1, -0.1, 1324.1324, -1324.1324};
...
static double edge_val[] = {-DBL_MIN, DBL_MIN, -DBL_MAX, DBL_MAX};

START_TEST(test_fabs_edge) {
...
Suite *suite_s21_fabs() {
  ...
  TCase *normal_case = tcase_create("s21_fabs_normal_val");
  tcase_add_loop_test(normal_case, test_fabs_normal, 0,
                      sizeof(normal_val) / sizeof(normal_val[0]));
  suite_add_tcase(suite, normal_case);

  TCase *edge_case = tcase_create("s21_fabs_edge_val");
  tcase_add_loop_test(edge_case, test_fabs_edge, 0,
                      sizeof(edge_val) / sizeof(edge_val[0]));
  suite_add_tcase(suite, edge_case);

  TCase *anomally_case = tcase_create("s21_fabs_anomally_val");
  tcase_add_loop_test(anomally_case, test_fabs_anomally, 0,
